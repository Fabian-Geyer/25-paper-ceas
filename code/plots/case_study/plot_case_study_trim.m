% Currently I do not know where the original simout data lies.
...
%% Select data
[filename,pathName] = uigetfile('.mat','Select SimOut File');
...
% qe = simLogs{6}.Values.ControlStates.error_attitude_quaternion_BR;
...
% Set up figure
f1 = figure(12345); clf;
set(f1, 'Color', 'w', 'Position', [100, 100, 1000, 600]); % Adjust for 2-column width
...
id0 = 10;
t = etas.Time(id0:end);
plot(t, rad2deg(etas.Data(10:end,2)), 'LineWidth', lineWidth, 'DisplayName', '\eta_{1}');
plot(t, rad2deg(etas.Data(id0:end,3)), 'LineWidth', lineWidth, 'DisplayName', '\eta_{2}');
plot(t, rad2deg(etas.Data(id0:end,4)), 'LineWidth', lineWidth, 'DisplayName', '\eta_{3}');
plot(t, rad2deg(etas.Data(id0:end,5)), 'LineWidth', lineWidth, 'DisplayName', '\eta_{4}');
grid on;
set(gca, 'FontName', fontName, 'FontSize', fontSizeTicks, 'LineWidth', axesLineWidth);
xlim([0 2000]);
ylim([-65, 65])
...
cleanfigure;
matlab2tikz('../../../paper/Figures/case_study_trim.tikz', 'width','15cm','height','7cm') % for small angle figure